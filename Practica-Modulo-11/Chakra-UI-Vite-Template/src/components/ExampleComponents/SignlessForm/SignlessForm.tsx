...
export const SignlessForm = ({ closeForm }: Props) => {
  ...
  const { sendMessageWithSignlessAccount, readState } = useContractUtils();
  const {
  ...
  const { generateNewVoucher, checkVoucherForUpdates, vouchersInContract } =
  ...
  const [
    ...
    setsectionConfirmCreationOfSignlessAccountIsOpen,
  ] = useState(false);
  const [noWalletAccountData, setNoWalletAccountData] =
    useState<FormDefaultValuesI>({
      accountName: "",
      password: "",
    });
  ...
  const handleConfirmData = async () => {
    const encryptedName = CryptoJs.SHA256(
  ...
  const handleSubmitNoWalletSignless = async ({
    accountName,
    password,
  }: FormDefaultValuesI) => {
    ...
    contractState = await readState(CONTRACT.programId, CONTRACT.metadata, {
      SignlessData: signlessAddress,
    });
  ...
  return (
    <Card>
        <Center>
    <Modal  heading="signless-form" close={closeForm}>
      ...
      {userHasWallet ? (
        ...
        >
          ...
          {sectionConfirmCreationOfSignlessAccountIsOpen && (
            ...
            >
              The account does not have a signless account, do you want to
              create one?
            </p>
          )}

          <Button className="signless-form__button" type="submit" >
            {!sectionConfirmCreationOfSignlessAccountIsOpen
              ? "Submit"
              : "Create"}
          </Button>
          ...
          {sectionConfirmCreationOfSignlessAccountIsOpen && (
            <Button
              ...
              onClick={() =>
                setsectionConfirmCreationOfSignlessAccountIsOpen(false)
              }
            >
              Cancel
            </Button>
